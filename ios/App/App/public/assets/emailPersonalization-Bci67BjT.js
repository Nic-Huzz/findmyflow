...
`;function u(o){const{offerName:t,confidence:e,flowType:n,funnelSteps:g,allScores:s}=o,c=e>=70?"Excellent Fit":e>=55?"Strong Fit":"Good Fit",r=new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"});return`
    <div class="pdf-container">
      ...
      <p class="pdf-date">Generated on ${r}</p>

      <div class="pdf-highlight-box">
        <div class="pdf-highlight-name">${t}</div>
        <div class="pdf-highlight-desc">${c} - ${e}% Match</div>
      </div>
    ...
    <div class="pdf-container">
      ...
      ${s?`
        <div class="pdf-section" style="background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.3);">
          ...
          <p style="font-style: italic; font-size: 18px; margin-bottom: 16px;">"${s}"</p>
          ${c?`<p style="color: rgba(255,255,255,0.8);">${c}</p>`:""}
        </div>
      `:""}
      ...
      ${r?`
        <div class="pdf-section" style="background: rgba(139, 92, 246, 0.1); border-color: rgba(139, 92, 246, 0.3);">
          ...
          <p style="white-space: pre-line; line-height: 1.6;">${r}</p>
        </div>
      `:""}
  ...
  `}function y(o,t="Vibe Rise Results"){const e=window.open("","_blank","width=800,height=600");if(!e){alert("Please allow popups to download your PDF");return}e.document.write(`
    <!DOCTYPE html>
    <html>
    <head>
      <title>${t}</title>
  ...
  `),e.document.close()}function v(o,t,e,n){var l;const s=((o==null?void 0:o.split("@")[0])||"").replace(/[._-]/g," ").split(" ").map(d=>d.charAt(0).toUpperCase()+d.slice(1).toLowerCase()).join(" "),c={attraction_offer:"Attraction Offer",upsell:"Upsell",downsell:"Downsell",continuity:"Continuity",leads_strategy:"Lead Generation",lead_magnet:"Lead Magnet"},r=[];return e&&Object.entries(e).forEach(([d,p])=>{p!=null&&p.label&&(d.includes("strength")||d.includes("best"))&&r.push(p.label)}),{name:s||"there",email:o,offer_type:c[t]||t,recommended_offer:((l=n==null?void 0:n.offer)==null?void 0:l.name)||"your offer",confidence:n!=null&&n.confidence?Math.round(n.confidence*100):null,key_strengths:r.slice(0,3),income_range:"$5K-20K/month",flow_completed_at:new Date().toISOString()}}function m(o,t,e){const g=((o==null?void 0:o.split("@")[0])||"").replace(/[._-]/g," ").split(" ").map(i=>i.charAt(0).toUpperCase()+i.slice(1).toLowerCase()).join(" "),s=i=>i?i>=1e6?`$${(i/1e6).toFixed(1).replace(/\.0$/,"")}M`:i>=1e3?`$${(i/1e3).toFixed(0)}K`:`$${i}`:"$100K",c=i=>i?i>=1e6?`${(i/1e6).toFixed(1).replace(/\.0$/,"")}M`:i>=1e3?`${(i/1e3).toFixed(0)}K`:i.toString():"1,000",r=t!=null&&t.impact_goal?parseInt(t.impact_goal.replace(/[^0-9]/g,""))-(t.being_seen_edge||0):0,l=t!=null&&t.income_goal?parseInt(t.income_goal.replace(/[^0-9]/g,""))-(t.earning_edge||0):0,d=r>l?"visibility":"earning",p=t!=null&&t.contracts_tested?Object.entries(t.contracts_tested).filter(([i,f])=>f==="yes").map(([i])=>i):[];return{name:g||"there",email:o,archetype:(e==null?void 0:e.archetype_name)||"Protective Pattern",archetype_description:(e==null?void 0:e.archetype_description)||"",edge_visibility:c(t==null?void 0:t.being_seen_edge),edge_earning:s(t==null?void 0:t.earning_edge),edge_type:d,core_fear:(e==null?void 0:e.core_fear)||"",active_contracts:p.slice(0,3),impact_goal:(t==null?void 0:t.impact_goal)||"",income_goal:(t==null?void 0:t.income_goal)||"",flow_completed_at:new Date().toISOString()}}export{x as a,m as b,y as d,v as e,u as g};
