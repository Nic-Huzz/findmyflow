...
// Helper function to format text with markdown and golden highlighting for dynamic insertions
function formatText(text, context = {}) {
  if (!text) return ''

  let formatted = text

  // First, identify and highlight dynamic insertions with golden color
  // Look for content that was inserted from context variables
  Object.keys(context).forEach(key => {
    const value = context[key]
    if (value && typeof value === 'string' && value.trim() !== '') {
      // Highlight the inserted values with golden color
      // Use a case-insensitive match but preserve original case
      const regex = new RegExp(`(${value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi')
      formatted = formatted.replace(regex, '<span style="color: #fbbf24; font-weight: 600;">$1</span>')
    }
  })
...
function HealingCompass() {
  ...
  return (
    <div className="app">
            ...
            <div key={message.id} className={`message ${message.isAI ? 'ai' : 'user'}`}>
              <div className="bubble">
                {message.kind === 'completion' ? (
                  <div className="text">
                    <div dangerouslySetInnerHTML={{ __html: formatText(message.text, context) }} />
                  ...
                  <div className="text" dangerouslySetInnerHTML={{ __html: formatText(message.text, context) }} />
                )}
      ...
      {/* Completion options (both buttons displayed together) */}
      {currentStep?.completion_options && currentStep.completion_options.length > 0 && (
        <div className="options-container">
          {currentStep.completion_options.map((option, index) => (
            <button
              key={index}
              className="option-button"
      ...
      {/* Static options */}
      {currentStep?.options && currentStep.options.length > 0 && !currentStep.options_from && !currentStep.completion_options && (
        <div className="options-container">
      ...
      {currentStep && !currentStep.options && !currentStep.options_from && !currentStep.completion_options && (
