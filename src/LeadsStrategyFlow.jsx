...
function LeadsStrategyFlow() {
    ...
    const loadData = async () => {
      try {
        const [questionsRes, offersRes] = await Promise.all([
          fetch('/leads-strategy-questions.json'),
          fetch('/leads-strategy-offers.json')
        ])
  ...
  const handleSaveResults = async () => {
    ...
    try {
      ...
      await supabase.from('leads_assessments').insert([{
        session_id: sessionId,
        user_id: user.id,
        user_name: user.user_metadata?.full_name || user.email?.split('@')[0] || 'User',
        email: user.email,
        responses: answers,
        recommended_strategy_id: recommendedOffer?.offer?.id,
        recommended_strategy_name: recommendedOffer?.offer?.name,
        confidence_score: recommendedOffer?.confidence,
        total_score: recommendedOffer?.totalScore,
        all_strategy_scores: allOfferScores.map(s => ({
          id: s.offer.id,
          name: s.offer.name,
          score: s.totalScore,
      ...
      try {
        await supabase.from('flow_sessions').insert({
          user_id: user.id,
          flow_type: '100m_leads',
          flow_version: 'leads-strategy-v1',
          status: 'completed',
      ...
      // Complete challenge quests (both Vibe Riser and Movement Maker use this flow)
      try {
        await completeFlowQuest({
          userId: user.id,
          flowId: 'flow_leads_strategy_vr',
          pointsEarned: 40
        })
        await completeFlowQuest({
          userId: user.id,
          flowId: 'flow_leads_strategy_mm',
          pointsEarned: 40
        })
      } catch (questError) {
  ...
  if (!questionsData || !offersData) {
    return (
      <div className="leads-strategy-flow">
