/**
 * Limiting Belief Rewire — Trace a safety contract back to its origin and heal it
 *
 * Originally Healing Compass V2. Renamed to clarify its purpose:
 *   1. Select a safety contract from Nervous System Map
 *   2. Describe how it limits you
 *   3. Trace it to a past experience
 *   4. Describe what happened
 *   5. Describe the emotions
 *   6. See how the belief formed (insight)
 *   7. Emotional splinter framing
 *   8. Healing options (challenge or 1-on-1)
 *
 * Saves to healing_compass_responses (same table as before)
 * Backward-compatible: ?results=true loads saved data
 */
...
export default function LimitingBeliefRewire() {
  const { user } = useAuth()
  ...
  const { saveProgress, loadProgress, clearProgress } = useAutoSave('limiting-belief-rewire', user?.id)
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          connect_dots_acknowledged: String(responses.connect_dots_acknowledged),
          splinter_removal_consent: String(responses.splinter_removal_consent),
      ...
      if (selectedOption === 'continue_challenge') {
        ...
        // Open Calendly in new tab, stay on current page
  ...
  const renderTimeCheck = () => (
    <div className="container welcome-container">
      <h1 className="welcome-greeting">Limiting Belief Rewire</h1>
  ...
  const renderQ8 = () => (
    <div className="ns-hc-container ns-hc-question-container">
      ...
      <FlowFeedback flowType="limiting_belief_rewire" userId={user?.id} />
  ...
  const renderNoResults = () => (
    <div className="ns-hc-container ns-hc-welcome-container">
      ...
      <div className="ns-hc-welcome-message">
        <p>It looks like you haven't completed the Limiting Belief Rewire yet.</p>
  ...
  const renderLocked = () => (
    <div className="ns-hc-container ns-hc-question-container">
      ...
      <div className="ns-hc-sticky-nav">
        ...
        <button className="ns-hc-secondary-button" onClick={() => navigate(-1)}>
          Go Back
        </button>
      </div>
    </div>
  ...
  return (
    <div className="ns-hc-app">
      {/* Progress Dots */}
      {currentScreen !== 'loading' && currentScreen !== 'no-results' && currentScreen !== 'locked' && (
      ...
      {currentScreen === 'locked' && renderLocked()}
