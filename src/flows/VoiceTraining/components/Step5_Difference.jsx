/**
 * Step 5: Your Difference (was Step 4: Unique Approach)
...
export default function Step5_Difference({ value, onChange, onContinue, voiceData }) {
  ...
  // Generate a voice preview based on data collected so far
...
Writing Samples: ${(voiceData?.contentSamples || []).filter(s => s.trim()).length} provided
Voice Influences: ${(voiceData?.voiceInfluences || []).filter(i => i.name.trim()).map(i => i.name).join(', ') || 'None'}
`
          ...
          {previewLoading ? (
            ...
            <>
              ...
              <div className="vt-preview-note">
                <span>✨</span>
                <p>This is based on your data so far. Complete the remaining steps for a fully personalized voice!</p>
              </div>
            </>
          )}
