...
const WINDOW_SIZE = 3
const MIN_CLASSIFICATIONS = WINDOW_SIZE * 2 // 6

/**
 * Returns the most frequent value in an array.
 * Ties broken by higher SHIFT_RANK (optimistic — celebrate growth).
 */
function mode(arr) {
  const counts = {}
  arr.forEach(v => { counts[v] = (counts[v] || 0) + 1 })
  return Object.entries(counts)
    .sort((a, b) => b[1] - a[1] || (SHIFT_RANK[b[0]] ?? -1) - (SHIFT_RANK[a[0]] ?? -1))
    [0]?.[0] || null
}
...
export async function detectShift(userId, questId, questLabel = null) {
  ...
  try {
    ...
    // 4. Compare first window vs last window
    const firstWindow = classifications.slice(0, WINDOW_SIZE)
    const lastWindow = classifications.slice(-WINDOW_SIZE)
    const fromState = mode(firstWindow)
    const toState = mode(lastWindow)
