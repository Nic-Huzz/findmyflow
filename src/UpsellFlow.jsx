...
function UpsellFlow() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setOffersData(offersJson.offers)
      } catch (err) {
  ...
  const calculateOfferScores = (userAnswers) => {
    ...
    const scores = offersData.map(offer => {
      let totalScore = 0
      const maxPossibleScore = offer.max_possible_score || 40

      // Calculate weighted score for each question
      Object.entries(userAnswers).forEach(([questionId, answer]) => {
        // Convert question ID format (q1_customer_base_size -> Q1_customer_base_size)
        const offerQuestionId = questionId.replace(/^q/, 'Q')
        const weights = offer.scoring_weights?.[offerQuestionId]
      ...
      <div className="upsell-flow">
        ...
        <div className="calculating-container">
          ...
          <div className="calculating-steps">
            <div className="calculating-step active">✓ Evaluating your customer base</div>
            <div className="calculating-step active">✓ Checking infrastructure & tracking</div>
            <div className="calculating-step active">✓ Scoring 4 upsell strategies</div>
    ...
    return (
      <div className="upsell-flow">
        ...
        <div className="reveal-container">
          ...
          {offer.tagline && <p className="reveal-tagline">{offer.tagline}</p>}
          ...
          <div className="funnel-preview">
            <h3 className="preview-heading">Implementation Steps:</h3>
          ...
          <div className="metrics-preview">
            <h3 className="preview-heading">Key Success Factors:</h3>
            <div className="metrics-grid">
              {offer.key_success_factors?.map((factor, index) => (
                <div key={index} className="metric-card">{factor}</div>
              ))}
            </div>
          </div>
    ...
    return (
      <div className="upsell-flow">
        <div className="success-container">
          ...
          <p>Your {recommendedOffer?.offer?.name} strategy is ready...</p>
          <div className="typing-indicator">
            <span></span><span></span><span></span>
          </div>
        </div>
