...
After event close-out, results appear one metric at a time with animated delays:
attendees → satisfaction → repeat rate → revenue.
**Files:** New `ExperienceResultsReveal.jsx` component, trigger on event completion
**Dependencies:** None
**Open question:** There is no explicit "close out event" flow currently. Events become "past" when their `experience_date` passes. The reveal would need to trigger on first view of a past event that hasn't been revealed yet (tracked in gamification state). Alternatively, add a "Close out event" button to the pipeline view that transitions the event to past + triggers the reveal.
...
### Sprint G15: Zarlo for Creator Portal (7 days)
...
Build creator page context map for `zarloPageContent.js`:
- Identity tab: playbook progress, positioning status, branch data
- Experiences tab: upcoming events, pipeline readiness, days until
- Growth tab: KPIs, Brand Pulse, 3% chain length

7 proactive bubble triggers:
1. Event 14d out, no attract content → "Want help writing your first post?"
2. Pipeline 80%+ → "Almost ready. What's left?"
3. No activity 7+ days → "Been quiet. When you're ready, your next step is [X]."
4. 3% improvement logged → "That's [N] in a row. You're compounding."
5. Sold-out event → "What did you do differently? Write it down."
6. Scale Score complete → "Here's what the highest-scoring creators have in common."
7. Quarterly planning empty → "New quarter. What are you running?"

**Files:** `zarloPageContent.js` (new creator entries), `CreatorHomeV2.jsx` (mount Zarlo widget)
**Dependencies:** Sprint G6 (Zarlo needs branch data for context)

### Sprint G16: AI Insight Drops (5 days)
AI-generated, not pre-written. Feed user context to Claude:
- Branch data (from monopoly finder)
- Playbook progress + completed flow data
- Courage challenge patterns (from interior scoreboard skill tags)
- 3% improvement themes
- DNA matches from 91-creator corpus

Insight types in the pool:
- DNA comparison ("Your rule break follows the same pattern as [Creator X]")
- Percentile ("Top 30% of creators who completed this")
- Monopoly update ("Healing confirmed as primary. Confidence 78%→82%")
- Pattern recognition ("Your 3% improvements have focused on marketing 4 times in a row")
- Playbook milestone ("Only 15% of creators get this far")

**UI:** Reuse `src/components/InsightDrop.jsx` slide-up animation (verified, exists). Consumer trigger logic in `src/hooks/useInsightDrops.js` is consumer-coupled (reads wahoo/streak data) and cannot be reused. Need new `useCreatorInsightDrops.js`.
**Data:** New edge function `generate-creator-insight` that gathers context + calls Claude
**Files:** New edge function, new `useCreatorInsightDrops.js` hook, mount in `CreatorHomeV2.jsx`
**Dependencies:** Sprint G6 (monopoly data), Sprint G15 (Zarlo can also surface insights)
